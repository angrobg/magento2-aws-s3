...
class StorageSyncCommand extends \Symfony\Component\Console\Command\Command
{
    ...
    protected function process(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->coreFileStorage->getCurrentStorageCode() == \ST\AwsS3\Model\MediaStorage\File\Storage::STORAGE_MEDIA_S3) {
        ...
        if ($this->coreFileStorage->getCurrentStorageCode() == \ST\AwsS3\Model\MediaStorage\File\Storage::STORAGE_MEDIA_FILE_SYSTEM) {
            try {
            ...
            $destinationModel = $this->coreFileStorage->getStorageModel(\ST\AwsS3\Model\MediaStorage\File\Storage::STORAGE_MEDIA_S3);
        ...
        if ($input->getOption('enable')) {
            ...
            $config->setDataByPath('system/media_storage_configuration/media_storage', \ST\AwsS3\Model\MediaStorage\File\Storage::STORAGE_MEDIA_S3);
            $config->save();
